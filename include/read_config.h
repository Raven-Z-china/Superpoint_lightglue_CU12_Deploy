...
struct Configs {
    ...
    Configs(const std::string &config_file, const std::string &model_dir) {
        ...
        YAML::Node superglue_node = file_node["superpoint_lightglue"];
        superpoint_lightglue_config.image_width = superglue_node["image_width"].as<int>();
        superpoint_lightglue_config.image_height = superglue_node["image_height"].as<int>();
        superpoint_lightglue_config.dla_core = superglue_node["dla_core"].as<int>();
        ...
        for (size_t i = 0; i < superglue_num_input_tensor_names; i++) {
            superpoint_lightglue_config.input_tensor_names.push_back(
        ...
        for (size_t i = 0; i < superglue_num_output_tensor_names; i++) {
            superpoint_lightglue_config.output_tensor_names.push_back(
