...
class LightGlue(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        if default_conf["input_dim"] != default_conf["descriptor_dim"]:
            self.input_proj = nn.Linear(default_conf["input_dim"], default_conf["descriptor_dim"], bias=True)
        else:
            self.input_proj = nn.Identity()

        head_dim = default_conf["descriptor_dim"] // default_conf["num_heads"]
        self.posenc = LearnableFourierPositionalEncoding(2, head_dim)

        h, n, d = default_conf["num_heads"], default_conf["n_layers"], default_conf["descriptor_dim"]
        ...
        state_dict = torch.load("../weights/superpoint_lightglue.pth", map_location="cpu")
    ...
    def forward(
        ...
        for i in range(default_conf["n_layers"]):
            # self+cross attention
            desc0, desc1 = self.transformers[i](desc0, desc1, encoding0, encoding1)
            if i == default_conf["n_layers"] - 1:
        ...
        return scores
        matches, mscores = filter_matches(scores, default_conf["filter_threshold"])
        mscores = torch.unsqueeze(mscores, 1)
        dmatch = torch.cat((matches, mscores), 1)
        return dmatch
        return matches, mscores
        # Skip unnecessary computation
        m0, m1, mscores0, mscores1 = filter_matches(scores, default_conf["filter_threshold"])
    ...
    def confidence_threshold(self, layer_index: int) -> float:
        ...
        threshold = 0.8 + 0.1 * np.exp(-4.0 * layer_index / default_conf["n_layers"])
    ...
    def get_pruning_mask(
        ...
        keep = scores > (1 - default_conf["width_confidence"])
    ...
    def check_if_stop(
        ...
        return ratio_confident > default_conf["depth_confidence"]
